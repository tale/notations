...
- (instancetype)initWithType:(int)type forController:(AppearanceSelectionTableCell *)controller {
	self = [super init];

  	if (self) {
		userDefaults = [[NSUserDefaults alloc] _initWithSuiteName:defaultsIdentifier container:[NSURL URLWithString:@"/var/mobile"]];
		...
		if (self.type == 1) {
			...
			self.iconImage = [UIImage imageWithContentsOfFile:thirdOptionImage];
			self.captionLabel.text = thirdOptionName;
		} else {
		...
		if ([appearanceStyle isEqualToNumber:[NSNumber numberWithInt:self.type]]) {
	  		[self.checkmarkButton setImage:[UIImage imageWithContentsOfFile:@"/Library/Application Support/Notations/selected.png"] forState:UIControlStateNormal];
		} else {
	  		[self.checkmarkButton setImage:[UIImage imageWithContentsOfFile:@"/Library/Application Support/Notations/unselected.png"] forState:UIControlStateNormal];
		}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier specifier:(PSSpecifier *)specifier {
	self = [super initWithStyle:style reuseIdentifier:reuseIdentifier specifier:specifier];
  	...
  	if (self) {
		...
		self.thirdStackView = [[AppearanceTypeStackView alloc] initWithType:2 forController:self];
		...
		[self.containerStackView addArrangedSubview:self.thirdStackView];
...
- (void)updateForType:(int)type {
	...
	AppearanceTypeStackView *toSelect;
  	...
  	if (type == 1) {
		notSelect1 = self.firstStackView;
		notSelect2 = self.thirdStackView;
		toSelect = self.secondStackView;
  	} else if (type == 2) {
		notSelect1 = self.firstStackView;
		notSelect2 = self.secondStackView;
		toSelect = self.thirdStackView;
	} else {
		notSelect1 = self.secondStackView;
		notSelect2 = self.thirdStackView;
	...
	[UIView transitionWithView:notSelect1.checkmarkButton duration:0.2f options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
		[notSelect1.checkmarkButton setImage:[UIImage imageWithContentsOfFile:@"/Library/Application Support/Notations/unselected.png"] forState:UIControlStateNormal];
	...
	[UIView transitionWithView:notSelect2.checkmarkButton duration:0.2f options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
		[notSelect2.checkmarkButton setImage:[UIImage imageWithContentsOfFile:@"/Library/Application Support/Notations/unselected.png"] forState:UIControlStateNormal];
	} completion:^(BOOL finished) {
		finished = YES;
	}];
  	...
  	[UIView transitionWithView:toSelect.checkmarkButton duration:0.2f options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
		[toSelect.checkmarkButton setImage:[UIImage imageWithContentsOfFile:@"/Library/Application Support/Notations/selected.png"] forState:UIControlStateNormal];
	} completion:^(BOOL finished) {
		finished = YES;
	}];
}
